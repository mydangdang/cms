...
| icon | VARCHAR | 50 | YES | NULL | 图标（Element Plus 图标名，仅菜单 type=2 使用，目录 type=1 不需要） |
...
字段使用说明：
- icon 字段仅对 type=2 (菜单) 有效，type=1 (目录) 不使用图标
- 顶级目录在 Header 顶部导航显示，仅显示标题
