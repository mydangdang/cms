...
│  ┌────────────────────────────────┐                        │
│  │ code: POST /admin/manager/add │                        │
│  └────────────────────────────────┘                        │
│                                                             │
└─────────────────────────────────────────────────────────────┘

字段使用说明：
- icon 字段对 type=1 (目录) 和 type=2 (菜单) 均可配置
- 顶级目录在 Header 顶部导航显示，可配置图标
- 二级/三级菜单在 Sidebar 侧边栏显示，包含图标
...
| 2 | 菜单 | {模块名}:{功能名} | system:manager |
| 3 | 按钮 | {模块名}:{功能名}:{操作} | system:manager:add |
| 4 | 接口 | {模块名}/{控制器名}/{方法名} | admin/manager/add |

**详细命名规则**：
- **目录（type=1）**：模块名称，如 `Workspace`、`System`
- **菜单（type=2）**：`模块名:功能名`，如 `system:manager`、`system:role`
- **按钮（type=3）**：`模块名:功能名:操作`，如 `system:manager:add`、`system:manager:edit`、`system:manager:delete`
- **接口（type=4）**：`模块名/控制器名/方法名`，如 `admin/manager/add`、`admin/role/edit`

**按钮 权限规则说明**：
- 格式：`{路由文件夹}:{路由页面}:{操作类型}`
- 路由文件夹：对应 src/views/ 目录（如 /src/views/system → system）
- 路由页面：对应路由path中页面（如 /system/manager → manager）
- 操作类型：对应操作（如 add、edit、delete）

**API 权限规则说明**：
- 格式：`{模块名}/{控制器名}/{方法名}`
- 模块名：对应 ThinkPHP 的模块目录（如 admin）
- 控制器名：对应控制器文件名（如 Manager.php → manager）
...
| 类型 | code 值 | 对应内容 |
|------|----------|----------|
| 菜单 | system:manager | 管理员管理页面 |
| 按钮 | system:manager:add | 新增按钮 |
| 接口 | admin/manager/add | POST /admin/manager/add |
...
> **说明**：使用 MySQL 用户变量 (@变量名) 实现父子级引用，避免因中间插入数据导致 parent_id 错乱。
> 执行前确保表为空，或根据实际情况调整。
...
-- 管理员 API 权限
INSERT INTO `bms_permissions` (`parent_id`, `title`, `code`, `type`, `sort_order`, `status`, `created_at`, `updated_at`) VALUES
(@manager_menu_id, '新增管理员API', 'admin/manager/add', 4, 4, 1, UNIX_TIMESTAMP(), UNIX_TIMESTAMP()),
(@manager_menu_id, '编辑管理员API', 'admin/manager/edit', 4, 5, 1, UNIX_TIMESTAMP(), UNIX_TIMESTAMP()),
(@manager_menu_id, '删除管理员API', 'admin/manager/delete', 4, 6, 1, UNIX_TIMESTAMP(), UNIX_TIMESTAMP());
